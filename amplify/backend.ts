...
userData.addCommands(
  ...
  '  # Use gemma2:9b (~5GB) instead of gemma3:27b (~14GB) to leave room for CLIP',
  '  OLLAMA_MODELS=$MOUNT_POINT/ollama ollama pull gemma2:9b',
  '  ',
  ...
  'fi',
  '',
  '# Ensure gemma2:9b is available (~5GB, leaves room for CLIP)',
  '# Pull is idempotent - skips if already present',
  'echo "Ensuring gemma2:9b model is available..."',
  'OLLAMA_MODELS=$MOUNT_POINT/ollama ollama pull gemma2:9b',
  '',
  '# Warm up the model',
  'echo "Warming up Gemma 2 model..."',
  'curl -s http://localhost:11434/api/generate -d \'{"model": "gemma2:9b", "prompt": "Hi", "stream": false}\' --max-time 180 || echo "Ollama warm-up done"',

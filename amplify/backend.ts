...
userData.addCommands(
  ...
  'fi',
  '',
  '# Set up symlinks for model storage (local filesystem, runs every boot)',
  ...
  '  ',
  '  # Set up SSH to use the deploy key from EFS',
  ...
  'else',
  '  echo "Warning: Git repo or deploy key not found"',
  '  echo "  REPO_DIR exists: $([ -d \"$REPO_DIR\" ] && echo yes || echo no)"',
  '  echo "  DEPLOY_KEY exists: $([ -f \"$DEPLOY_KEY\" ] && echo yes || echo no)"',
  '  echo "Using existing code on EFS"',
  'fi',
  '',
